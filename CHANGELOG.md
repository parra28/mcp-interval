...
### Added
- `icu_create_training_plan` — new all-in-one tool that creates a `PLAN` folder and bulk-uploads all of its workouts and notes in a single call. It computes each item's `day` offset from the plan's earliest date, maps note `distance` (phase length in days) to the API's `days` field, attaches the `folder_id`, and validates required fields (`category`, `start_date_local`, `name`) up front — reporting missing data as `validation_error` suggestions instead of uploading a partial plan. Backed by new `ICUClient.create_folder` and `ICUClient.create_multiple_workouts` methods.
...
- **Breaking — response shape:** `icu_get_activity_details` restructured its output into named sections (replacing the previous flat/grouped hybrid). Fields previously in `power`, `heart_rate`, `cadence`, `training`, and `other` are now in richer sections with additional fields. New sections `power_model`, `zones`, `altitude`, `weather`, `flags`, `achievements`, and `analysis_meta` have no prior equivalent.
