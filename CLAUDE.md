...
MCP (Model Context Protocol) server for Intervals.icu — provides up to 83 tools, 4 resources, and 9 prompts for accessing training data, wellness metrics, and performance analysis through Claude and other LLMs. The default `INTERVALS_ICU_DELETE_MODE=safe` registers 80 tools; `full` registers all 83, `none` registers 76.
